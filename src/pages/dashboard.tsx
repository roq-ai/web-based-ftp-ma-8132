...
import { FtpAccountListPage as ShowCaseList } from 'pages/ftp-accounts';
...
function DashboardPage() {
  ...
  const ftpAccountStat = useSWR<PaginatedInterface<FtpAccountInterface>>(
    () => `/ftp-accounts`,
    () => getFtpAccounts({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Ftp Accounts',
      result: ftpAccountStat?.data?.totalCount,
      isLoading: ftpAccountStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`ftp-accounts`} passHref legacyBehavior>
                  <Button
